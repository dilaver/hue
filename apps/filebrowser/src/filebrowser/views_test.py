...
def test_view_avro():
  ...
  try:
    ...
    cluster.fs.chown('/test-avro-filebrowser', 'test')
...
def test_view_parquet():
  ...
  try:
    ...
    cluster.fs.chown('/test-parquet-filebrowser/', 'test')
...
def test_view_gz():
  ...
  try:
    ...
    if cluster.fs.isdir("/test-gz-filebrowser"):
      cluster.fs.do_as_superuser(cluster.fs.rmtree, '/test-gz-filebrowser/')

    cluster.fs.mkdir('/test-gz-filebrowser/')
    cluster.fs.chown('/test-gz-filebrowser/', 'test')
...
def test_view_i18n():
  ...
  try:
    ...
    try:
      cluster.fs.do_as_superuser(cluster.fs.rmtree, '/test-filebrowser/')
    except Exception, ex:
...
def view_helper(cluster, encoding, content):
  ...
  try:
    f = cluster.fs.do_as_superuser(cluster.fs.open, filename, "w")
    f.write(bytestring)
    f.close()
    ...
    try:
      cluster.fs.do_as_superuser(cluster.fs.remove, filename)
    except Exception, ex:
...
def test_trash():
  ...
  try:
    ...
    if cluster.fs.isdir('/user/%s/.Trash' % USERNAME):
      assert_equal([('http://testserver/filebrowser/view/user/test/.Trash', 302)], response.redirect_chain)
    else:
